...
import { ITimesheetsService } from 'domain/timesheets'
...
export class LeaveService implements ILeaveService {
	...
	async create(leave: any): Promise<ResponseModel<{ created: boolean }>> {
		try {
			const leaveData = leave?.Entry?.AnswersJson?.page1
			const leavePayload = {
				staffId: Number(leaveData?.staffID) || null,
				type: leaveData?.leaveType || '',
				startDate: leaveData?.firstDayFormatted || '',
				endDate: leaveData?.lastDayFormatted || '',
				comments: leaveData?.comments || '',
				totalDays: Number(leaveData?.days) || 0,
				status: 'Pending',
				approvedBy: '',
			}
			console.log('LEAVE', leavePayload)

			const { created, id } = await this.leaveRepository.create(leavePayload)
			if (!created) {
