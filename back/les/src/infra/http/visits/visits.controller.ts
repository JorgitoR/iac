...
export class VisitsController {
	...
	@Put('pre-start')
	@ApiOperation({ summary: 'Pre start visit' })
	@ApiResponse({
		status: 200,
		description: 'Pre start created',
	})
	@ApiResponse({
		status: 404,
		description: 'Pre start not created',
	})
	@ApiResponse({
		status: 401,
		description: 'Unauthorized, The token provided is invalid',
	...
	@HttpCode(200)
	async preStart(@Body() visit: any): Promise<
		ResponseModel<{
			created: boolean
		}>
	> {
		const response = await this.visitsService.preStart(visit)
		return response
	}
	@Put('close-day')
	@ApiOperation({ summary: 'Close day visit' })
	@ApiResponse({
		status: 200,
		description: 'Close day created',
	})
	@ApiResponse({
		status: 404,
		description: 'Close day not created',
	})
	@ApiResponse({
		status: 401,
		description: 'Unauthorized, The token provided is invalid',
	...
	@HttpCode(200)
	async closeDay(@Body() visit: any): Promise<
		ResponseModel<{
			created: boolean
		}>
	> {
		console.log("ENTROOPO",visit)
		const response = await this.visitsService.closeDay(visit)
		return response
	}
