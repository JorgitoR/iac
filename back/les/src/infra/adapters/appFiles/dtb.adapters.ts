...
export class AppFilesDtb implements IAppFilesRepository {
	...
	async updateByFileID(file: any, link: string): Promise<{ updated: boolean }> {
		try {
			const db = await this.dtb.getSequelizeInstance()
			const appFile = await db.model(AppFilesDb).findOne({
				where: {
					file_name: file.originalname.replace(/\.[^.]+$/, ''),
				},
			})
			if (appFile == null) {
				return Promise.resolve({
			...
			await appFile.update({ link:link } as any)
			return { updated: true }
		} catch (error) {
			return Promise.resolve({
				updated: false,
